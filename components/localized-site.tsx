...
import { ArrowLeft, ArrowRight, ArrowUpRight, Binoculars, Caravan, CalendarDays, Check, CircleDollarSign, Clock, CloudSun, Heart, Layers, ListChecks, Map, MapPin, MapPinned, Menu, Navigation, Route, Search, ShieldCheck, SlidersHorizontal, TentTree, Trees } from "lucide-react";
...
function LocalizedCard({ article, locale, priority = false }: { article: Article; locale: LocaleCode; priority?: boolean }) {
  ...
  return <Link className={`article-card ${priority ? "article-card-featured" : ""}`} href={prefix(locale,`/guides/${article.slug}`)}><div className="article-image" style={{backgroundImage:`linear-gradient(180deg, transparent 35%, rgba(16,26,13,.45)), url(${article.image})`}}><span>{label}</span></div><div className="article-card-copy"><div className="eyebrow"><MapPin size={13}/>{article.region} · {article.readTime} min</div><h3>{article.title}</h3><p>{article.description}</p><span className="text-link">{ui.readGuide} <ArrowUpRight size={15}/></span></div></Link>;
}
...
export function LocalizedHome({ locale }: { locale: LocaleCode }) {
  ...
  return <>
    ...
    <main>
      <section className="hero"><div className="hero-glow"/><div className="hero-grid"><div className="hero-copy"><span className="kicker"><TentTree size={14}/> {ui.heroKicker}</span><h1>{ui.heroTitle}</h1><p>{ui.heroText}</p><div className="hero-actions"><div className="download-card"><div className="qr-image" style={{backgroundImage:"url(/images/kiwicamping-qr.png)"}}/><StoreLink locale={locale}/></div><Button asChild variant="outline" size="lg" className="w-[170px] rounded-[14px]"><Link href={`${prefix(locale)}#features`}>{ui.seeHow}<ArrowRight/></Link></Button></div><div className="hero-proof"><span><strong>{ui.statPlaces}</strong><small>{ui.statPlacesLabel}</small></span><span><strong>{ui.statOffline}</strong><small>{ui.statOfflineLabel}</small></span><span><strong>{ui.statStays}</strong><small>{ui.statStaysLabel}</small></span></div></div><div className="hero-visual"><div className="image-slot hero-image-slot" style={{backgroundImage:"url(/images/kiwicamping-hero.webp)"}}/></div></div></section>
